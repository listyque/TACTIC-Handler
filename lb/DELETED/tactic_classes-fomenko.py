...
def get_server():
    return server_auth(env.Env().get_server(), env.Env().get_project(), env.Env().get_user(), env.Env().get_user())
...
def server_restart(first_run=False):
    ...
    if reply == QtGui.QMessageBox.NoRole:
        if env.Mode().get == 'maya':
            ...
            msg = QtGui.QMessageBox(QtGui.QMessageBox.Warning, 'Failed to run TACTIC Handler', 'TACTIC Server not running!', QtGui.QMessageBox.NoButton, env.Inst().ui_main)
...
class SObject(object):
    ...
    def update_snapshots(self):
        ...
        self.get_snapshots(snapshot_dict)
    ...
    def get_tasks(self):
        self.tasks = self.query_tasks(self.info['code'])
...
# def query_all_sobject_tasks(asset_code):
#     search_type = 'sthpw/task'
#     filters = [('search_code', asset_code), ('project_code', env.Env().get_project())]
#
#     tasks = server_query(search_type, filters)
#     from pprint import pprint
#     pprint(tasks)
#     return tasks


def tasks_query(asset_code, process, user=None):
    """
    Query for Task
    ...
    """
    search_type = 'sthpw/task'
    filters = [('search_code', asset_code), ('process', process), ('project_code', env.Env().get_project())]
...
def checkin_playblast(snapshot_code, file_name):
    ...
    playblast = server_start().add_file(snapshot_code, file_name, file_type='playblast', mode='move', create_icon=False,
