...
// Qt Menu owns native mouse-grab forwarding between parent and child windows.
// Placement and lifecycle are shared with Popup through PopupLogic.
QtControls.Menu {
    id: control
    delegate: QtControls.MenuItem { visible: false }
    ...
    padding: 6
    // Native cascade positions outer windows; overlap only their shadow gutters.
    overlap: 2 * effectiveShadowMargin
    modal: false
    ...
    function openAsSubmenu(sourceItem, logicalAnchor) {
        if (!sourceItem)
            return
        restoreOwnerActivationOnClose = false
        openingAnchorItem = logicalAnchor || sourceItem
        protectOpeningFrom(openingAnchorItem)
        const edge = sourceItem.mapToGlobal(sourceItem.width, 0)
        const area = availableGeometryAt(edge.x, edge.y)
        if (area) {
