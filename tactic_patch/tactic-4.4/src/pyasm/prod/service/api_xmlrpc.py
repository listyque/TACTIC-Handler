...
class ApiXMLRPC(BaseApiXMLRPC):
    ...
    @xmlrpc_decorator
    def get_virtual_snapshot_path(my, ticket, search_key, context, snapshot_type="file", is_revision=False, level_key=None, file_type='main', file_name='', mkdirs=False, protocol='client_repo', ext='', checkin_type=''):
    ...
    @xmlrpc_decorator
    def get_virtual_snapshot_extended(my, ticket, search_key, context, snapshot_type="file", is_revision=False,
                                  level_key=None, file_type='main', file_name='', mkdirs=False, protocol='client_repo',
                                  ext='', checkin_type='', version=None):
        '''creates a virtual snapshot and returns a path that this snapshot
        would generate through the naming conventions''
        ...
        # create a virtual snapshot
        snapshot = Snapshot.create(sobject, snapshot_type=snapshot_type, context=context, description=description,
                                   is_revision=is_revision, level_type=level_type, level_id=level_id, commit=False,
                                   version=version)

        relative_path = snapshot.get_dir('relative', file_type=file_type)
        ...
        non_relative_path_part = correct_path.replace(filename, '').replace(relative_path, '').replace('//', '/')

        return_dict = {
            'left_part_path': non_relative_path_part,
    ...
    def create_snapshot(my, ticket, search_key, context, snapshot_type="file",
        ...
        snapshot = Snapshot.create(sobject, snapshot_type=snapshot_type, context=context, description=description, is_current=is_current, level_type=level_type, level_id=level_id, is_revision=is_revision, triggers=triggers, version=version)
    ...
    def add_file(my, ticket, snapshot_code, file_path, file_type='main', use_handoff_dir=False, mode=None, create_icon=False, dir_naming=None, file_naming=None, checkin_type='strict', custom_repo_path=None, do_update_versionless=True, subdir=None):
        ...
        for i, file_path in enumerate(file_paths):
            ...
            if mode == 'preallocate':
                ...
                if custom_repo_path:
                    lib_dir = "%s/%s" % (custom_repo_path, snapshot.get_relative_dir(file_type=file_type, file_object=file_object))
                else:
            ...
            checkin = FileAppendCheckin(snapshot_code, sub_file_paths, sub_file_types, keep_file_name=keep_file_name, mode=mode, source_paths=source_paths, dir_naming=dir_naming, file_naming=file_naming, checkin_type=checkin_type, do_update_versionless=do_update_versionless, custom_repo_path=custom_repo_path)
            checkin.execute()
            snapshot = checkin.get_snapshot()
