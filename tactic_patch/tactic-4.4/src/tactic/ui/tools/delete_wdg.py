...
class DeleteCmd(Command):
    ...
    def delete_sobject(my, sobject):
        ...
        files_list = values.get("files_list")
            ...
            for related_type in related_types:
                ...
                for related_sobject in related_sobjects:
                    if related_type == 'sthpw/snapshot':
                        my.delete_snapshot(related_sobject, files_list)
                    else:
        ...
        if search_type == 'sthpw/snapshot':
            my.delete_snapshot(sobject, files_list)
        else:
            sobject.delete()


    
    def delete_snapshot(my, snapshot, files_list=None):

        if files_list:
            # print files_list

            file_paths = files_list.get('file_path')
            print 'file_paths', file_paths

            files_skeys = files_list.get('search_key')
            files = snapshot.get_related_sobjects("sthpw/file")
            print 'files', files

            for i, file_skey in enumerate(files_skeys):
                for file in files:
                    if file_skey == file.get_search_key():
                        print "deleting file: ", file_skey
                        file.delete()

            for file_path in file_paths:
                print "removing path: ", file_path
                if platform.system() == 'Linux':
                    # if tatic started on linux paths always starts from single '/'
                    if file_path[0] == '/':
                        FileUndo.remove(file_path)
                else:
                    # if windows, then all paths started from '/' should be network, else drive letter
                    if file_path[0] == '/':
                        FileUndo.remove('/' + file_path)
                    else:
                        FileUndo.remove(file_path)

            delete_snapshot = files_list.get('delete_snapshot')
            print 'delete_snapshot', delete_snapshot
            if delete_snapshot:
